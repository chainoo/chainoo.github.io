...
def sort_time(stime):
    data = None
    try:
        data = datetime.strptime(stime, "%Y-%m-%dT%H:%M:%Sz")
    except Exception as e:
        print(stime, e)
        data = datetime.strptime("1970-01-01T00:00:00z", "%Y-%m-%dT%H:%M:%Sz")
    return data
  ...
  repository(owner: "%s", name: "%s") {
    refs(refPrefix: "refs/tags/", first: 3, orderBy: {field: TAG_COMMIT_DATE, direction: DESC}) {
      edges {
        ...
        resp_data = resp.json()['data']['repository']['refs']['edges']
        #print("[resp_data] %s" %resp_data)
        #print("[post_data] %s" %post_data)
        if resp_data == []:
            data['name'] = "Not Found"
            data['tag_name'] = ""
            data['html_url'] = "https://github.com/%s" % pro['repo']
            data['body'] = ""
            data['created_at'] = pro['add_time']
        else:
            try:
...
#result['updated_at'] = datetime.fromtimestamp(time.mktime(now.timetuple())).isoformat() + 'Z'
if result['data']:
...
print("[Result total] %s" %result['total'])
