<template>
  <div class="home">
    <div class="header">
      <div class="container">
        <span class="title"><a href="./">Chain-Version</a></span>
        <span class="title-desc"> 列出区块链源码当前版本 </span>
      </div>
    </div>
    <div class="main">
      <a-back-top />
            ...
            <a-list-item slot="renderItem"
              ...
              <a-list-item-meta>
                <div slot="description">
                  <span class="tag-title">浏览器：</span>
                  <a class="ant-tag ant-tag-has-color"
                          style="background-color: #8CD7B0;"
                          target="_blank"
                          :href="item.explorer">Explorer</a>
                  <span class="tag-title">文档：</span>
                  <a class="ant-tag ant-tag-has-color"
                          style="background-color: #8CD9CA;"
                          target="_blank"
                          :href="item.doc">部署</a>
                  <a class="ant-tag ant-tag-has-color"
                  ...
                  <a-tooltip placement="top"
                    ...
                    <a-tag color="#2db7f5"
                           @click="tagClick(item.project, item.body)">{{ item.name.substr(0,40) || item.tag_name.substr(0,38) }}</a-tag>
                  </a-tooltip>
                  <a-tooltip placement="top"
                             title="Last 10">
                    <a-tag color="#5bd1e7"
                  ...
                  <span class="tag-title">发布时间：</span>
                  <a-tag color="#F17F00">{{ item.created_at || "None" }}</a-tag>
      ...
      <div class="footer">
        <p></p>
        <span>blockchain-version ©2019 Created by Ericyao</span>
      </div>
...
export default {
  ...
  data () {
    return {
      tags: ['utxo', 'bitcoin', 'ethereum', 'eos', 'cosmos', 'polkadot', 'nervos'],
      listData: [],
      updated_at: '',
      total: '',
      search_text: '',
      ...
      trending: false,
      trendData: [],
      runDay: parseInt((new Date().getTime() - new Date(Date.parse('2019-12-01'.replace(/-/g, '/'))).getTime()) / 1000 / 3600 / 24)
    }
  },
  components: {
    AButton: Button,
